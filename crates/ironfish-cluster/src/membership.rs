...
impl MembershipManager {
    ...
    pub async fn join(&self, request: JoinRequest) -> Result<JoinResponse> {
        let is_leader = self.local_node.is_leader().await;

        if !is_leader {
            return Ok(JoinResponse {
                accepted: false,
                leader_id: self.local_node.leader().await,
                members: Vec::new(),
    ...
    pub async fn cluster_status(&self) -> ClusterStatus {
        let members = self.members.read().await;
        let local_status = self.local_node.status(members.len() + 1).await;
        ...
        ClusterStatus {
            nodes,
            leader: self.local_node.leader().await,
            term: self.local_node.term(),
            healthy: true,
        }
    }
