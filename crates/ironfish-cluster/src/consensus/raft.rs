...
impl RaftConsensus {
        ...
        loop {
            tokio::select! {
                _ = heartbeat_timer.tick() => {
                    if !matches!(self.node.state().await, NodeState::Leader) {
                        break;
                    }
    ...
    async fn start_election(&self) -> Result<bool> {
        ...
        self.node.set_state(NodeState::Candidate).await;
        ...
        *self.voted_for.write().await = Some(self.node.id().clone());
        ...
        if votes_received >= votes_needed {
            ...
            self.node.set_state(NodeState::Follower).await;
            Ok(false)
        }
    }
    ...
    async fn become_leader(&self) {
        ...
        self.node.set_state(NodeState::Leader).await;
        self.node.set_leader(Some(self.node.id().clone())).await;
        ...
        tokio::spawn(async move {
            ...
            loop {
                timer.tick().await;
                if !matches!(node.state().await, NodeState::Leader) {
                    break;
                }
            }
        });
...
impl ConsensusProtocol for RaftConsensus {
    ...
    async fn stop(&self) -> Result<()> {
        ...
        self.node.set_state(NodeState::Leaving).await;
    ...
    async fn append_entries(&self, request: HeartbeatRequest) -> Result<HeartbeatResponse> {
        ...
        if request.term < current_term {
            return Ok(HeartbeatResponse {
                ...
                term: current_term,
                success: false,
                metrics: self.node.metrics().await,
            });
        }
        ...
        self.node.set_state(NodeState::Follower).await;
        self.node.set_leader(Some(request.leader_id.clone())).await;
    ...
    fn state(&self) -> NodeState {
        futures::executor::block_on(self.node.state())
    }
    ...
    fn leader(&self) -> Option<NodeId> {
        futures::executor::block_on(self.node.leader())
    }
}
